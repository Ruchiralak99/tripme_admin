  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
      <div class="flex items-center gap-3 mb-2">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-slate-900">Edit Aircraft</h1>
      </div>
      <p class="text-slate-600 mt-1">Update aircraft information and images.</p>
    </div>
  </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <form method="POST" action="{{ route('super_admin.aircrafts.update', $aircraft->id) }}" class="p-6" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number"
                       ...
                       value="{{ old('passenger_seats', $aircraft->passenger_seats) }}"
                       min="1"
                       class="block w-full px-3 py-3 border border-slate-300 rounded-lg shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
            ...
            <div class="lg:col-span-2">
                ...
                <textarea id="overview"
                          ...
                          required>{{ old('overview', $aircraft->overview) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select id="status"
                        ...
                        required>
                    <option value="active" {{ old('status', $aircraft->status) === 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ old('status', $aircraft->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
            ...
            <div class="lg:col-span-2 border-t border-slate-200 pt-6 mt-6">
                <h4 class="text-lg font-medium text-slate-900 mb-4">Current Images</h4>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach($aircraft->images as $index => $image)
                    <div class="relative group">
                        <img src="{{ asset('storage/' . $image) }}" alt="Aircraft image {{ $index + 1 }}" class="w-full h-24 object-cover rounded-lg border border-slate-200">
                        <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-200 rounded-lg flex items-center justify-center">
                            <span class="text-white text-xs opacity-0 group-hover:opacity-100">Image {{ $index + 1 }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
                <p class="text-sm text-slate-500 mt-2">Add new images below to append to existing ones, or leave empty to keep current images only.</p>
            </div>
            @endif

            {{-- Add New Images --}}
            <div class="lg:col-span-2 {{ $aircraft->images && count($aircraft->images) > 0 ? '' : 'border-t border-slate-200 pt-6 mt-6' }}">
                @if(!($aircraft->images && count($aircraft->images) > 0))
            ...
            <div class="lg:col-span-2">
                <label for="images" class="block text-sm font-medium text-slate-700 mb-2">
                    {{ $aircraft->images && count($aircraft->images) > 0 ? 'Add More Images' : 'Upload Images' }}
                </label>
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-lg hover:border-slate-400 transition-colors">
                    <div class="space-y-1 text-center w-full">
                        ...
                        <p class="text-xs text-slate-500">{{ $aircraft->images && count($aircraft->images) > 0 ? 'New images will be added to existing ones' : 'You can select multiple images at once' }}</p>
                    </div>
                </div>
