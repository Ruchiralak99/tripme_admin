...
<div class="mb-8">
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
      <h1 class="text-2xl sm:text-3xl font-bold text-slate-900">Create New User</h1>
      <p class="text-slate-600 mt-1">Add a new user to the system with appropriate role and permissions</p>
    </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <form method="POST" action="{{ route('super_admin.user_store') }}" class="p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="role" id="role" required onchange="toggleVendorType()"
                    ...
                    <option value="payment_officer" {{ old('role') == 'payment_officer' ? 'selected' : '' }}>Payment Officer</option>
                    <option value="booking_officer" {{ old('role') == 'booking_officer' ? 'selected' : '' }}>Booking Officer</option>
                    <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>User</option>
                </select>
            </div>

            {{-- Vendor Type --}}
            <div id="vendorTypeContainer" style="display: {{ old('role') == 'vendor' ? 'block' : 'none' }};">
            ...
            {{-- Password Section --}}
            <div class="lg:col-span-2 pt-6 border-t border-slate-200">
                <h4 class="text-lg font-medium text-slate-900 mb-4">Account Security</h4>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 pt-8 border-t border-slate-200 mt-8">
            ...
            <a href="{{ route('super_admin.user_management') }}" class="inline-flex items-center justify-center px-6 py-3 bg-slate-500 text-white rounded-lg hover:bg-slate-600 transition-colors duration-200 font-medium">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
