...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center space-x-4">
            ...
            <h1 class="text-2xl font-bold text-slate-900">Edit Promo Code</h1>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-lg shadow-sm border border-slate-200">
        <form method="POST" action="{{ route('super_admin.promo_codes.update', $promoCode->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="p-6">
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="author" name="author" value="{{ old('author', $promoCode->author) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="discount_type" name="discount_type" required
                                ...
                                <option value="fixed" {{ old('discount_type', $promoCode->discount_type) == 'fixed' ? 'selected' : '' }}>Fixed Amount (LKR)</option>
                            </select>
                            @error('discount_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="discount_value" name="discount_value" value="{{ old('discount_value', $promoCode->discount_value) }}"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="minimum_amount" name="minimum_amount" value="{{ old('minimum_amount', $promoCode->minimum_amount) }}"
                                       step="0.01" min="0"
                                       class="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                                       placeholder="0.00">
                            </div>
                        ...
                        <div id="maximum_discount_field" style="display: none;">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-2 text-slate-500">LKR</span>
                                <input type="number" id="maximum_discount" name="maximum_discount" value="{{ old('maximum_discount', $promoCode->maximum_discount) }}"
                                       step="0.01" min="0"
                                       class="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                                       placeholder="0.00">
                            </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" id="usage_limit" name="usage_limit" value="{{ old('usage_limit', $promoCode->usage_limit) }}"
                                   min="1"
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                            ...
                            <p class="mt-1 text-xs text-blue-600">Current usage: {{ $promoCode->used_count }} times</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="valid_from" name="valid_from" value="{{ old('valid_from', $promoCode->valid_from->format('Y-m-d')) }}"
                                   required
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                            @error('valid_from')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="date" id="valid_until" name="valid_until" value="{{ old('valid_until', $promoCode->valid_until->format('Y-m-d')) }}"
                                   required
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                            @error('valid_until')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-8 bg-slate-50 rounded-lg p-4">
                    <h3 class="text-lg font-semibold text-slate-900 mb-4">Current Usage Statistics</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="text-center">
                            <div class="text-2xl font-bold text-blue-600">{{ $promoCode->used_count }}</div>
                            <div class="text-sm text-slate-600">Times Used</div>
                        </div>
                        @if($promoCode->usage_limit)
                            <div class="text-center">
                                <div class="text-2xl font-bold text-green-600">{{ $promoCode->usage_limit - $promoCode->used_count }}</div>
                                <div class="text-sm text-slate-600">Remaining Uses</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-orange-600">{{ number_format(($promoCode->used_count / $promoCode->usage_limit) * 100, 1) }}%</div>
                                <div class="text-sm text-slate-600">Usage Rate</div>
                            </div>
                        @else
                            <div class="text-center">
                                <div class="text-2xl font-bold text-green-600">∞</div>
                                <div class="text-sm text-slate-600">Unlimited Uses</div>
                            </div>
                        @endif
                    </div>
                </div>
