...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
        <form method="POST" action="{{ route('super_admin.packages.rides.book.store') }}" enctype="multipart/form-data" id="bookingForm">
            @csrf
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
                ...
                <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                    @if($ride->image_path)
                        <img src="{{ asset('storage/' . $ride->image_path) }}" alt="{{ $ride->name }}" class="w-16 h-16 rounded-lg object-cover">
                    @else
                    ...
                    <div class="text-right">
                        <div class="text-lg font-bold text-primary-600">Rs. {{ number_format($ride->price_lkr, 2) }}</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="full_name" id="full_name" value="{{ old('full_name') }}"
                               class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="city_id" id="city_id" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
                ...
                <div class="space-y-4">
                    <label class="flex items-start gap-3 p-4 border border-slate-300 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                        <input type="radio" name="payment_type" value="tentative" class="mt-1" checked onchange="updatePaymentSection()">
                        <div class="flex-1">
                            <div class="font-medium text-slate-900">Tentative Booking</div>
                            <div class="text-sm text-slate-600">Reserve your spot without payment. Confirm later with the team.</div>
                            <div class="text-sm font-medium text-green-600 mt-1">No payment required</div>
                        </div>
                    </label>

                    <label class="flex items-start gap-3 p-4 border border-slate-300 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                        <input type="radio" name="payment_type" value="partial" class="mt-1" onchange="updatePaymentSection()">
                        <div class="flex-1">
                    ...
                    <label class="flex items-start gap-3 p-4 border border-slate-300 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                        <input type="radio" name="payment_type" value="full" class="mt-1" onchange="updatePaymentSection()">
                        <div class="flex-1">
                ...
                <div id="payment-section" class="mt-6 hidden">
                    <div class="border-t border-slate-200 pt-6">
                        ...
                        <div id="partial-amount-section" class="hidden mb-4">
                            ...
                            <input type="number" name="partial_amount" id="partial_amount" min="5000" step="0.01"
                                   class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-6">
            ...
            <div class="space-y-3 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">Rs. <span id="base-price">{{ number_format($ride->price_lkr, 2) }}</span></span>
                </div>
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">Rs. <span id="subtotal">{{ number_format($ride->price_lkr, 2) }}</span></span>
                </div>

                <div class="flex justify-between">
                    <span class="text-slate-600">Tax ({{ $ride->tax_percentage }}%):</span>
                    <span class="font-medium">Rs. <span id="tax-amount">{{ number_format(($ride->price_lkr * $ride->tax_percentage) / 100, 2) }}</span></span>
                </div>
                ...
                <div id="full-payment-discount-row" class="flex justify-between hidden">
                ...
                <div class="border-t border-slate-200 pt-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-xl font-bold text-primary-600">Rs. <span id="total-amount">{{ number_format($ride->price_lkr + (($ride->price_lkr * $ride->tax_percentage) / 100), 2) }}</span></span>
                    </div>
                </div>
...
function updatePriceSummary() {
    ...
    document.getElementById('tax-amount').textContent = taxAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    ...
    if (appliedPromoCode) {
        ...
        document.getElementById('promo-discount').textContent = promoDiscount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    } else {
    ...
    if (paymentType === 'full') {
        ...
        document.getElementById('full-payment-discount').textContent = fullPaymentDiscount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    } else {
...
function updatePaymentAmount(total) {
    ...
    if (paymentType === 'tentative') {
        ...
        paymentAmountSection.classList.remove('hidden');
        document.getElementById('payment-amount').textContent = total.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    } else if (paymentType === 'partial') {
        paymentAmountSection.classList.remove('hidden');
        const partialAmount = parseFloat(document.getElementById('partial_amount')?.value) || 5000;
        document.getElementById('payment-amount').textContent = partialAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    }
}
...
function validatePromoCode() {
    ...
    .then(data => {
        if (data.valid) {
            ...
            messageDiv.innerHTML = `<span class="text-green-600">✓ Promo code applied! You save Rs. ${data.discount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>`;
            updatePriceSummary();
        } else {
